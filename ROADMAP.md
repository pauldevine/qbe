...
3. **Bare-metal output:** ✅ DONE 2026-06-11 (§6c) — `omf_link.py --raw-binary --load-addr 0x3000` (link-time-absolute selectors, synthesized register-setup stub at the image head), minic-built crt0 (`minic/dos/newlibc/bm_crt0.c`) + polled-serial console (`bm_console.c`, pure volatile-far MMIO — no inline asm needed), and `hello_bm` printing PASS over serial on MAME victor9k via the Lua loader (`tools/build-newlibc-baremetal.sh` + `tools/run-victor-baremetal.sh`); gated in `test_omf_link.sh` (structure) and `tools/test-victor.sh` (golden diff).
4. **Drivers + ISRs:** ⏳ step 4a DONE 2026-06-11 (§6d) — the interrupt-handler strategy is a **compiler feature**: `__attribute__((interrupt))` travels as QBE `interrupt` function linkage and the i8086 backend emits the full ES-safe ISR ABI itself (static CS-local ES save — the hardware-validated Victor pattern, all-register save, DS/ES=DGROUP from a link-time `dw DGROUP` word, iret); `isr_probe` gates it DOS-hosted (small+medium) and `timer_bm` runs LIVE 8253-ch2/IR2 timer interrupts through a minic-built ISR on MAME victor9k (`bm_pic.c` full 8259A re-init + `bm_interrupts.c` + `bm_timer.c`, zero inline asm).  Step 4b DONE 2026-06-12 (§6e) — display (6845 + font RAM + glyph-pointer VRAM), keyboard (VIA-CS2 state machine, interrupt-driven on IR6), and serial RX (7201 channel B on IR1, a real ISR where newlibc's was a stub) all run bare-metal through compiler-emitted ISRs; `display_bm`/`keyboard_bm`/`serial_bm` PASS on MAME with harness keystroke (`V9K_KEYPOST`) and serial-byte (`V9K_SERIAL_IN`) injection.  **Extended-asm verdict: NOT needed** — the drivers' remaining inline asm was ia16-gcc ES damage control and a pushf/cli flags-save the ISR-only ring-buffer design obsoletes.  Step 4c DONE 2026-06-12 (§6f) — `memory_bm`/`crtc_bm`/`pic_bm`/`interrupt_bm` ported (the last is the cross-driver stress: display init + 60 scrolling lines under the live timer ISR), every port first-run PASS, zero compiler changes; only driver delta is `bm_pic_get_mask`/`set_mask`.  Remaining 4d: cooked console (display+keyboard behind getc/putc), block/SASI port.
5. **Harness adoption:** ⏳ STARTED 2026-06-12 (§6e) — `tools/test-newlibc.sh` is the standing bare-metal battery (9/9 as of §6f: hello, timer, display, keyboard, serial, memory, crtc, pic, interrupt-stability), golden-diffed per test; the DOS-hosted newlibc tests remain `test-dos.sh` gate entries.  Grow it as more phase-3 tests port over.
