...
**The original 4-phase plan is done, and so is the MicroPython hardening phase (Phase 5).** From ~2026-05-28 to 2026-06-11 the project ported **MicroPython** as a real-world stress test on real Victor 9000 hardware: each failure was reduced to a focused `minic/dos/examples/*_probe.c`, the underlying compiler/backend/runtime bug was fixed, and the probe was added to the `tools/test-dos.sh` gate (now **262/262**). MicroPython is feature-complete-enough and is retained as a **regression corpus** — after toolchain changes, the 108-TU compact build is rebuilt and byte-compared; Victor re-runs happen only when bytes move. **Phase 6 — adopting `~/projects/newlibc` as the real C library and test harness — is the designated next frontier (2026-06-11).** Day-to-day status lives in `CLAUDE.md` and `NEXT_SESSION.md`; this file is the high-level reconciliation.
...
1. **Survey + triage session:** pick the DOS-hosted phase 1 tests as the entry point (they run under our existing DOSBox/MAME harnesses with no bare-metal work); inventory compiler-dialect gaps by compiling phase 1/phase 3 sources with minic and bucketing the failures (the MicroPython-spike methodology — `run-codegen.sh` style per-TU sweep).
2. **Portable-subset milestone:** compile the toolchain-neutral subsets (string/printf wrappers, VFS, FAT) with minic; run their tests DOS-hosted in DOSBox; add gate entries.
