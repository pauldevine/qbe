...
**Last Updated:** 2026-05-25 (hh — track (h) closed: new `Oloadfl`/`Ostorefl` backend ops + minic.y `loadfar`/`storefar` wiring + lexer K regen. `long` through a far pointer now round-trips full 32 bits under compact/large/huge instead of truncating to 16. storefl_probe (5 asserts × 3 models) pins. Closes [[storefar-lacks-storefl]]; gate 56/56)
**Status:** ~97% Complete
