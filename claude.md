...
**Last Updated:** 2026-05-25 (dd — emit_push_long CAddr-safe; Kl Odiv/Orem with `(unsigned long)&sym / k` now routes relocations through omf_link; pinned by caddr_div_probe across compact/large/huge; gate 47/47)
**Status:** ~96% Complete
...
- Latent Kl-CAddr arith — **closed 2026-05-25 (aa/bb/cc/dd)**: 141f2e8 fixed Oloadf*/Ostoref*/Oadd/Osub; (bb) fixed Oand/Oor/Oxor; (cc) fixed cmp32_high/cmp32_low which covers all 10 Kl Oc*l comparison handlers; (dd) fixed emit_push_long which covers Odiv/Orem Kl (signed + unsigned).  Ocopy was already CAddr-safe.  Omul Kl with CAddr is unreachable from C semantics.
