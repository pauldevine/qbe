...
| 1b — Aggregate/designated initializers | ⛔ Still gated, but now measurable | The `dataitem()` emitter design (approved plan §1a).  Config layer now exists so the spike can finally rank it; current top blockers (array typedef, comma-expr) sit *ahead* of it in the convergence chain — clear those first, then re-scope §1b from the next tally. |
