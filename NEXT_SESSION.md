...
- NEW dos_file_probe.c: fopen w + fputs/fputc, fclose, fopen r + getc to EOF,
  remove, reopen.  4-way gate (small/medium x {libstub anchor, libstub-free}),
  one golden, all first-run identical.  test-dos 344 -> 348.
- stevie TFILE.TXT in DOSBox now loads the file (status `"TFILE.TXT" 3 lines,
  34 characters`, content rendered) — the exact bug fixed.  Existing probes
  (printf_nolibstub, dos_libc) stay golden on the dos_vfs stack.

### ⇒ Next session (§7t)
- Deeper stevie verify (edit -> :w -> reload, multi-file, tags).
