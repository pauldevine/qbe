#!/bin/bash
# Build divmod32_probe.exe — standalone DOS .EXE that tests qbe i8086
# 32-bit divide/remainder lowering and the _qbe_div32u/s / _qbe_rem32u/s
# libstub soft helpers.  Modeled on tools/build-sprintf-probe.sh.
#
# Usage: tools/build-divmod32-probe.sh
# Output: build/divmod32_probe/divmod32_probe.exe

set -eu

QBE_DIR="$(cd "$(dirname "$0")/.." && pwd)"
SRC="$QBE_DIR/stevie-orig/divmod32_probe.c"
OUT_DIR="$QBE_DIR/build/divmod32_probe"
...
cpp -P -nostdinc -isysroot/var/empty -DDOS -D__TURBOC__ \
	"-I$INC_DIR" "-I$QBE_DIR/stevie-orig" \
...
# Stage 3: ASM normalize (same sed/awk/perl pipeline as build-stevie.sh).
prefix="${base}_"
...
"$QBE_DIR/tools/omf_link.py" \
	-o "$OUT_DIR/divmod32_probe.exe" \
	--map "$OUT_DIR/divmod32_probe.map" \
	--entry _start \
	--stack-size 8192 \
...
echo "  OK: $OUT_DIR/divmod32_probe.exe ($(wc -c <"$OUT_DIR/divmod32_probe.exe") bytes)"
