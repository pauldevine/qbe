#!/bin/bash
# Build int86x_probe.exe — standalone DOS .EXE that tests the libstub
# segment-aware DOS API trio (_int86x / _intdosx / _segread).
# Modeled on tools/build-divmod32-probe.sh.
#
# Usage: tools/build-int86x-probe.sh
# Output: build/int86x_probe/int86x_probe.exe

set -eu

QBE_DIR="$(cd "$(dirname "$0")/.." && pwd)"
SRC="$QBE_DIR/stevie-orig/int86x_probe.c"
OUT_DIR="$QBE_DIR/build/int86x_probe"
...
"$QBE_DIR/tools/omf_link.py" \
	-o "$OUT_DIR/int86x_probe.exe" \
	--map "$OUT_DIR/int86x_probe.map" \
	--entry _start \
	--stack-size 8192 \
...
echo "  OK: $OUT_DIR/int86x_probe.exe ($(wc -c <"$OUT_DIR/int86x_probe.exe") bytes)"
