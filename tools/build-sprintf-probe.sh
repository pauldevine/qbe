#!/bin/bash
# Build sprintf_probe.exe — standalone DOS .EXE that tests libstub's sprintf.
#
# Mirrors the .EXE link path in build-stevie.sh but limited to one source
# (stevie-orig/sprintf_probe.c) plus crt0_exe + libstub.
#
# Usage: tools/build-sprintf-probe.sh
# Output: build/sprintf_probe/sprintf_probe.exe

set -eu

QBE_DIR="$(cd "$(dirname "$0")/.." && pwd)"
SRC="$QBE_DIR/stevie-orig/sprintf_probe.c"
OUT_DIR="$QBE_DIR/build/sprintf_probe"
...
"$QBE_DIR/tools/omf_link.py" \
	-o "$OUT_DIR/sprintf_probe.exe" \
	--map "$OUT_DIR/sprintf_probe.map" \
	--entry _start \
	--stack-size 8192 \
...
echo "  OK: $OUT_DIR/sprintf_probe.exe ($(wc -c <"$OUT_DIR/sprintf_probe.exe") bytes)"
