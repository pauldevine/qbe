 ...
 * $Log:	version.c,v $
 *
 * v3.69b 90/04/20 Toad Hall Tweak
 * - Referred to throughout as "v1.1"
 * - Nothing significant: only patches to reduce/eliminate Turbo C errors
 *   and warnings.
 * - Compiling interactively.  See TCONFIG.TC for setup.
 *
 * Revision 3.69a  90/02/04            dave
 * A couple of minor bugfixes, and a whole load of enhancements:
 *   -	Added tag-stacking.
 *   -	Added searches for sentences and paragraphs.
 *   -	Enhancements to "help" command:
 *	 . Navigation of help screens, not just next-screen.
 *	 . Index of help screens.
 *	 . Screen for MSDOS-specific features.
 *	 . Screen for :set commands.
 *	 . A little context-sensitivity.
 *   -	File mode is preserved (it wasn't in previous versions).
 *   -	UNIX-like specifications of environments variables are expanded
 *	in file names.  (E.g.- you can say  :e $HOME/autoexec.bat)
 *   -	PC keys for Insert & Delete now work.
 *   -	Ported to Turbo C for the PC.
 *   -	Integrated Larry Shurr's BIOS option for the PC.
 *   -	Added color and multi-line options for the PC, and hooks for them
 *	in other systems.
 *   -	Made "ctags" a little more general.
 *
